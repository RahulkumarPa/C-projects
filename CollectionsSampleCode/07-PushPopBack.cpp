...
class WholeNumbers
{
	...
	public:
		...
		void PushBack(int newValue); // NEW! for adding an element to the back/end
		int PopBack();				 // NEW! for removing an element from the back/end
}; 
...
int main()
{
	try
	{
		...
		for(int i = 0; i < 4; i++)
		{
			...
			numbersObj.PushBack((i + 1) * 5); // abritrary values: 5, 10, 15, 20
		}
		...
		while(numbersObj.Size() >= 0)
		{
			...
			cout << "\n\'Popping\': " << numbersObj.PopBack() << endl;
		}
...
// NEW! for adding an element to the back/end
...
// NEW! for removing an element from the back/end
