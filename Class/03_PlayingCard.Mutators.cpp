/** PlayingCard.Mutators.cpp
 *	
 *	In this example program, we did the following: 
 *		- Added mutators to the class.
 *   
 *	@author		<YOUR-NAME>
...
int main()
{
	...
	try
	{
		...
		testCard.IsFaceUp = true; // make it face-up
		testCard.SetRank("Ace", true); // set the rank to "Ace"
		// set the suit to "Spades"
		
		// Display the card
		cout << "\ntestCard rank is " << testCard.GetRank() << " and suit is ";
		cout << "\ntestCard value is " << testCard.GetSuit(); 
   	    cout << "\ntestCard as a string is "<< testCard.ToString() << endl;
   		
   		// More testing
   		//testCard.SetValue(11); // set the value to 11
		//testCard.FlipCard(); // flip the card
   		//testCard.SetRank("Blackjack", true); // set the rank to "Blackjack"
		//testCard.SetSuit("Clovers"); // set the suit to "Clovers"
   		// Display the card again
   		//cout << "\ntestCard is now: "<< testCard.ToString() << endl;
